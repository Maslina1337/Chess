...
function checkWin() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        for (let j = 0; j < 8; j++) {
            if (chessField[i][j] !== 0 && chessField[i][j] !== "wk" && chessField[i][j] !== "bk") {
                onlyKings = false;
                break;
            }
        }
        ...
        moves[(String(y)+String(x))].map((i, yi) => {
            i.map((j, xj) => {
                if (j === 1) {
                    ...
                    if (userOptionalRights.visible_moves[0]) {
                    ...
                    let place = window.document.getElementById(chessFieldPlaces[yi][xj]);
                    place.style.cursor = "pointer";
                    if (userOptionalRights.visible_beat_moves[0]) {
...
function selectFigureSmart(y, x) {
    ...
    if (userRights.canSelectFigure) {
        ...
        place.getElementsByClassName("blaze")[0].style.border = `11px solid ${userOptionalRights.color_select[0]}`;
